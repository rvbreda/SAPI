...
class SystemTasksController extends AppController {
    ...
    private function setProjectStatus($projectId) {
        ...
        if($project['Project']['final_score'] >= 80){
            ...
            $this->sendDirectorEmailProjectEvaluated($project['Project']['id']);
        }else{
    ...
    public function sendDirectorEmailProjectEvaluated($projectId) {
                try {
                    //SELECIONA O PROJETO
                    $this->loadModel('Project');
                    $this->loadModel('User');
                    ...
                    //SELECIONA O EMAIL DO DIRETOR
                    $direc = $this->User->find('first', array(
            ...
            $email  ->from('neurobase@pucrs.br', 'Instituto do Cérebro PUCRS')
                    ->to($direc['User']['email'])
                    ->subject('Projeto Avaliado')
                    ->template('directornewproject', 'inscer')
                    ->emailFormat('both');
            $email->viewVars(array('projectName' => $project['Proposal']['description'], 'projectId' => $projectId));
